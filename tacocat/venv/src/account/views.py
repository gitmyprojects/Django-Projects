...
def account_search_view(request, *args, **kwargs):
    ...
    if request.method == "GET":
        ...
        if len(search_query) > 0:
            # look for the field 'email' in Account. the i in icontains means dont consider case
...
def account_view(request, *args, **kwargs):
    ...
    if account:
        # these are the variable that will pass into the template
        ...
        is_friend = False
        user = request.user
        # is the user authenticated and viewing someone elses account
